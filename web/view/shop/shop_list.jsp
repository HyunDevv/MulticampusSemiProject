...
function setMarkers(arrs,arrs2) {
   $(arrs).each(
               function(index, shop) {
                  ...
                  marker.setMap(map);
                  var iwContent = '<div style="padding:5px; width:120px; font-size:small;"><a href="shop_detail.mc?shopid='+shop.shopid+'" style="color:blue; font-size:large;" target="_blank">' + shop.shopname + '</a>'+ ' 추천수(' + arrs2[index].up + ')' + '<br>'+shop.shopphonenumber+'</div>' 
                              ,iwRemoveable = true;
            ...
            function(index, list) {
               ...
               if(sw==0){
                  ...
                  if(longiA[0]+range > list.lat && longiA[0]-range < list.lat && latiA[0]+range > list.lon && latiA[0]-range < list.lon){
                     result += '<tr><td>'+list.shopid+'</td><td><img id="img1" src="img/'+list.img1+'"></td><td><a href="shop_detail.mc?shopid='+list.shopid+'">'+list.shopname+'</a></td><td>'+list.userid+'</td><td>'+list.cnt+'</td></tr>';
                     $('table').append(result);
                  }                  
               }else{
